...
class BloodDropsStateWidget extends StatelessWidget {
  BloodLevelsModel type;

  BloodDropsStateWidget(this.type);

  List<String> bloodImages = <String>[
    'images/alarmB.png',
    'images/very_lowB.png',
    'images/lowB.png',
    'images/mediumB.png',
    'images/highB.png'
  ];
  List<String> bloodAmount = <String>[
    'stan alarmowy',
    'stan bardzo niski',
    'stan niski',
    'stan średni',
  ...
  List<String> loadBloodState(String bloodType) {
    ...
    if (bloodType == 'A Rh+') {
      ...
      check = checkBloodAmount(type.aRh_minus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == 'B Rh+') {
      check = checkBloodAmount(type.bRh_plus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == 'B Rh-') {
      check = checkBloodAmount(type.bRh_minus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == 'AB Rh+') {
      check = checkBloodAmount(type.abRh_plus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == 'AB Rh-') {
      check = checkBloodAmount(type.abRh_minus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == '0 Rh+') {
      check = checkBloodAmount(type.zeroRh_plus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    } else if (bloodType == '0 Rh-') {
      check = checkBloodAmount(type.zeroRh_minus);
      list.add(bloodImages[check]);
      list.add(bloodAmount[check]);
    }
    return list;
